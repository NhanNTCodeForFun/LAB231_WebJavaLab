...
<html>
    ...
    <body>
        ...
        <form action="DispatchController">
            ...
            <a href="LoadAllPostServlet">Home</a>
        ...
        <c:if test="${not empty listPost}">
            <c:forEach var="post" items="${sessionScope.LIST_POST}" varStatus="counter">
                ...
                <c:if test="${post.isYourPost}">
                    ...
                    <a href="DeletePostServlet?&txtPostId=${post.postDTO.postId}" onclick="return confirm('Are you sure you want to delete this post?');">Delete Post</a>
                </c:if>

            </c:forEach>
        </c:if>
        <c:set var="numberOfPage" value="${sessionScope.NUMBER_OF_PAGE}"/>
        <form action="ShowPostServlet">
            <c:forEach begin="1" end="${numberOfPage}" varStatus="counter">
                
                <c:if test="${sessionScope.CURRENT_PAGE == counter.count}">
                    <input type="submit" value="${counter.count}" name="btPage" style="color: red"/>
                </c:if>
                <c:if test="${sessionScope.CURRENT_PAGE != counter.count}">
                    <input type="submit" value="${counter.count}" name="btPage"  />
